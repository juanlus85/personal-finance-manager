...
import { UserAccessManager } from "@/components/finance/UserAccessManager";
...
function SettingsContent() {
  ...
  return <div className="space-y-6 lg:space-y-8"><section><Badge variant="outline" className="rounded-full border-[#CBDDD4] bg-[#ECF5F0] text-[#235545] font-medium">Preferencias de la aplicación</Badge><h1 className="font-display text-3xl sm:text-4xl font-semibold tracking-[-0.04em] mt-3">Configuración</h1><p className="text-muted-foreground mt-2 leading-6 max-w-2xl">Personaliza categorías, tipos de cambio y los parámetros que aseguran un seguimiento financiero consistente.</p></section>
    ...
    <section className="grid gap-5 xl:grid-cols-2"><Card className="card-elevated"><CardHeader className="p-5 sm:p-6"><CardTitle className="font-display text-xl">Acceso y privacidad</CardTitle><p className="text-sm text-muted-foreground mt-1">Las cuentas de acceso se almacenan en la base de datos de forma independiente de los movimientos financieros.</p></CardHeader><CardContent className="p-5 sm:p-6 pt-0 space-y-4"><div className="flex items-start gap-3 rounded-xl bg-[#EAF4EC] p-4"><ShieldCheck className="h-5 w-5 text-[#276548] shrink-0 mt-0.5" /><div><p className="text-sm font-medium">Acceso con usuarios locales</p><p className="text-xs text-muted-foreground mt-1">Las contraseñas se guardan con hash seguro y cada usuario accede únicamente a sus propios datos financieros.</p></div></div><div className="flex items-start gap-3 rounded-xl bg-secondary p-4"><KeyRound className="h-5 w-5 text-primary shrink-0 mt-0.5" /><div><p className="text-sm font-medium">Sesiones protegidas</p><p className="text-xs text-muted-foreground mt-1">El secreto de sesión sigue protegido en el VPS; las credenciales no se incluyen en copias JSON ni en el repositorio.</p></div></div></CardContent></Card><Card className="card-elevated"><CardHeader className="p-5 sm:p-6"><CardTitle className="font-display text-xl">Información de la aplicación</CardTitle><p className="text-sm text-muted-foreground mt-1">Referencia para comprobar la versión desplegada.</p></CardHeader><CardContent className="p-5 sm:p-6 pt-0"><div className="rounded-xl border border-border p-4"><p className="text-xs uppercase tracking-[0.13em] text-muted-foreground">Versión y compilación</p><p className="font-mono-finance text-sm mt-2">Versión {BUILD_LABEL}</p><p className="text-xs text-muted-foreground mt-2">Zona horaria de negocio: Europe/Madrid (España).</p></div></CardContent></Card></section>
    {user?.role === "admin" ? <UserAccessManager /> : null}
