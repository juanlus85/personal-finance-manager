...
const kindLabels: Record<string, string> = {
  ...
  possible_expense: "Gasto posible",
  extra_bill: "Recibo extraordinario",
  card_expense: "Gasto de tarjeta",
  card_forecast: "Previsión de tarjeta",
...
function MovementsContent() {
  ...
  const [forecastIssuer, setForecastIssuer] = useState<string | null>(null);
  ...
  return <div className="space-y-6 lg:space-y-8">
    ...
    {transactionKind ? <TransactionDialog open={Boolean(transactionKind)} onOpenChange={open => { if (!open) { setTransactionKind(null); setSelectedTransaction(null); setForecastIssuer(null); } }} kind={transactionKind} transaction={selectedTransaction} initialDate={transactionKind === "card_forecast" ? `${nextMonth(month)}-01` : undefined} initialDescription={transactionKind === "card_forecast" && forecastIssuer ? `${forecastIssuer} · pago previsto` : undefined} /> : null}
