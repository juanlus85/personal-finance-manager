...
const portableBackupSchema = z.object({
  ...
  data: z.object({
    ...
    financings: z.array(z.object({ id: importedIdSchema, concept: z.string().min(1).max(180), provider: importedNullableText(160), currency: currencySchema, monthlyAmount: importedDecimalSchema.refine(value => value > 0), totalAmount: importedDecimalSchema.nullable().optional().default(null), paymentDay: z.number().int().min(1).max(31), startDate: dateSchema, endDate: dateSchema, status: z.enum(["active", "archived"]), notes: importedNullableText(4_000) })).max(5_000).default([]),
    recurringTransactions: z.array(z.object({ id: importedIdSchema, categoryId: importedIdSchema.nullable().optional().default(null), accountId: importedIdSchema.nullable().optional().default(null), name: z.string().min(1).max(180), direction: directionSchema, kind: z.enum(["fixed_income", "recurring_bill"]), certainty: z.enum(["confirmed", "possible"]), currency: currencySchema, amount: importedDecimalSchema.refine(value => value > 0), dayOfMonth: z.number().int().min(1).max(31), startDate: dateSchema, endDate: dateSchema.nullable().optional().default(null), notes: importedNullableText(4_000), isActive: z.boolean() })).max(10_000).default([]),
    transactions: z.array(z.object({ id: importedIdSchema, categoryId: importedIdSchema.nullable().optional().default(null), accountId: importedIdSchema.nullable().optional().default(null), description: z.string().min(1).max(220), direction: directionSchema, kind: z.enum(["extra_income", "possible_income", "possible_expense", "extra_bill", "card_expense", "card_forecast", "manual_income", "manual_expense"]), certainty: z.enum(["confirmed", "possible"]), currency: currencySchema, amount: importedDecimalSchema.refine(value => value > 0), exchangeRateToEur: importedDecimalSchema.nullable().optional().default(null), effectiveDate: dateSchema, notes: importedNullableText(4_000) })).max(20_000).default([]),
...
export const financeRouter = router({
  ...
  importData: protectedProcedure.input(z.object({ backup: z.unknown() })).mutation(async ({ ctx, input }) => {
    ...
    return db.transaction(async tx => {
      ...
      const financingIdMap = new Map<number, number>();
      ...
      for (const financing of backup.financings) {
        const result = await tx.insert(financings).values({ userId: ctx.user.id, concept: financing.concept, provider: financing.provider, currency: financing.currency, monthlyAmount: financing.monthlyAmount.toFixed(2), totalAmount: financing.totalAmount === null ? null : financing.totalAmount.toFixed(2), paymentDay: financing.paymentDay, startDate: financing.startDate, endDate: financing.endDate, status: financing.status, notes: financing.notes });
        financingIdMap.set(financing.id, Number(result[0].insertId));
      }
      for (const recurring of backup.recurringTransactions) {
        const result = await tx.insert(recurringTransactions).values({ userId: ctx.user.id, categoryId: recurring.categoryId ? categoryIdMap.get(recurring.categoryId) ?? null : null, accountId: recurring.accountId ? accountIdMap.get(recurring.accountId) ?? null : null, name: recurring.name, direction: recurring.direction, kind: recurring.kind, certainty: recurring.direction === "expense" ? "confirmed" : recurring.certainty, currency: recurring.currency, amount: recurring.amount.toFixed(2), dayOfMonth: recurring.dayOfMonth, startDate: recurring.startDate, endDate: recurring.endDate, notes: recurring.notes, isActive: recurring.isActive });
        recurringIdMap.set(recurring.id, Number(result[0].insertId));
      }
      for (const transaction of backup.transactions) {
        const result = await tx.insert(transactions).values({ userId: ctx.user.id, categoryId: transaction.categoryId ? categoryIdMap.get(transaction.categoryId) ?? null : null, accountId: transaction.accountId ? accountIdMap.get(transaction.accountId) ?? null : null, description: transaction.description, direction: transaction.direction, kind: transaction.kind, certainty: transaction.kind === "possible_income" || transaction.kind === "possible_expense" ? "possible" : transaction.direction === "expense" ? "confirmed" : transaction.certainty, currency: transaction.currency, amount: transaction.amount.toFixed(2), exchangeRateToEur: transaction.currency === "USD" && transaction.exchangeRateToEur ? transaction.exchangeRateToEur.toFixed(8) : null, effectiveDate: transaction.effectiveDate, notes: transaction.notes });
        transactionIdMap.set(transaction.id, Number(result[0].insertId));
      }
      for (const settlement of backup.monthlyConceptSettlements) {
        const conceptMatch = /^(loan|financing|recurring|transaction)-(\d+)$/.exec(settlement.conceptId);
        const sourceId = conceptMatch ? Number(conceptMatch[2]) : null;
        const mappedId = !conceptMatch || sourceId === null ? null : ({
          loan: loanIdMap,
          financing: financingIdMap,
          recurring: recurringIdMap,
          transaction: transactionIdMap,
        }[conceptMatch[1]]?.get(sourceId) ?? null);
        const conceptId = conceptMatch && mappedId ? `${conceptMatch[1]}-${mappedId}` : settlement.conceptId;
